...
pub struct WithdrawGem<'info> {
    ...
    /// CHECK: we MUST ask for this PDA both during deposit and withdrawal for sec reasons, even if it's zero'ed
    #[account(seeds = [
            b"gem_rarity".as_ref(),
            bank.key().as_ref(),
            gem_mint.key().as_ref()
    ...
    /// CHECK: unlike with deposits, the gem can be sent out to anyone, not just the owner
    #[account(mut)]
