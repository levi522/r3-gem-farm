...
function App() {
  ...
  async function getStakedNfts() {
    ...
    console.log("farmStarted: ", farmStarted)
    const farmerStarted = await fetchFarmer(connection, wallet)
    console.log("farmerStarted: ", farmerStarted)
  ...
  async function stakeNft1(nft) {
    const connection = new Connection(clusterApiUrl(env.solana_network), opts.preflightCommitment);
    console.log("stakeNft1 nft", nft)
  ...
  async function stakeMoreNfts(nft) {
    ...
    console.log("nft",nft);
    console.log("nft nft.mint",nft.mint.toBase58())
  ...
  async function stakeMoreNfts1(nft) {
    const connection = new Connection(clusterApiUrl(env.solana_network), opts.preflightCommitment);
    console.log("stakeMoreNfts1",nft);
    const stakeResult = await stakerMoreMover(nft, connection, wallet)
    console.log("stakeResult",stakeResult)
    const farmerStarted = await fetchFarmer(connection, wallet)
    console.log("farmerStarted",farmerStarted)
  ...
  if (!wallet.connected) {
    ...
    <div className="App">
      ...
      <div>
            ...
            (value && farmerState === "unstaked") ? (
              <>
                ...
                <div className="m-1 card flex justify-center">
                  ...
                  <button onClick={() => stakeNft1(value)}>Stake all</button>
                </div>
            ...
            (farmerState == "staked" && stakedValue && value) ? (
              <>
                ...
                <div className="m-1 card flex justify-center">
                  ...
                  <button onClick={() => stakeMoreNfts1(value)}>all Stake</button>
                </div>
                {console.log("state: ", farmerState)}
                <h2>Your Staked NFTs</h2>
                <div>
        ...
        {
          (farmerState == "pendingCooldown" && stakedValue && value) ? (
              <>
                <button onClick={refreshAll}>Refresh NFTs</button>
                <h2>Reward Value: {rewardValue}</h2>
                <button onClick={claimRewards}>Claim Rewards</button>
