...
export class GemFarmClient extends GemBankClient {
  ...
  async flashDeposit(
    ...
    const signers = [];
    if (isKp(farmerIdentity))
      signers.push(<Keypair>farmerIdentity);
  ...
  async getInitFarmerInstruction(
      farm: PublicKey,
      bank: PublicKey,
      farmerIdentity: PublicKey | Keypair,
      farmerPDA: PublicKey,
  ...
  async getDepositGemInstruction(bank: PublicKey, //: PublicKey,
                                 vault: PublicKey, //PublicKey,
                                 vaultOwner: PublicKey | Keypair, //PublicKey | Keypair,
                                 gemAmount: BN, //: BN,
                                 gemMint: PublicKey ,   //:  nft.mint
                                 gemSource: PublicKey) { // nft.pubkey
    const [gemBox, gemBoxBump] = await this.findGemBoxPDA(vault, gemMint);
    const [GDR, GDRBump] = await this.findGdrPDA(vault, gemMint);
    const [vaultAuth, vaultAuthBump] = await this.findVaultAuthorityPDA(vault);
  ...
  async getStakeInstruction(
      ...
      bank: PublicKey,
      farmerIdentity: PublicKey | Keypair, //: PublicKey | Keypair,
      farmerPDA: PublicKey,
      farmAuth: PublicKey,
      farmAuthBump: number,
      farmerBump: number,
      farmAccBank: PublicKey,
      vault: PublicKey) {
